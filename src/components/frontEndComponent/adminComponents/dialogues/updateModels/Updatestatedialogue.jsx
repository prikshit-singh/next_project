...
export default function Createstatedialogue(props) {
  ...
  useEffect(()=>{
    if(props.data !== null){
      ...
      setStateCode(props.data.statecode)
    }
   },[props.data])
 ...
 const handleSubmit = async () => {
  ...
  if (statecode == '') {
    ...
    const data = {ID:props.data._id,title,statecode}
    
    try {

        const res = await axios.post(`${apis.baseUrl}${apis.updateState}`, data, {
          headers: {
        ...
        if (res.data.CODE === 200) {
          props.afterUpdate()
            toast('State updated successfully', { hideProgressBar: false, autoClose: 2000, type: 'success' })
            handleClose()
        } else {
            setLoader(false)
      ...
      >
        <Box sx={style}>
          ...
          <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between', }}>
            <h2 style={{
              color: 'var(--primary)',
            }} id="parent-modal-title">Update State</h2>
            <DoneIcon
