...
export default function Updatecoursedialogue(props) {
  ...
  useEffect(() => {
    if (props.data !== null) {
      ...
      setCourseCode(props.data.coursecode)
      setDuration(props.data.duration)
      const mapData = props.data.subject.map((data) => data._id)
  ...
  const handleSubmit = async () => {
    ...
    const newSubject = subject.map(data => data._id)
    const data = { ID: props.data._id, title, coursecode, subject: newSubject, duration }
    console.log(newSubject, subject)

    try {

      const res = await axios.post(`${apis.baseUrl}${apis.updateCourse}`, data, {
        headers: {
          'token': session.data ? session.data.userData.token : '',
        }
      })
      if (res.data.CODE === 200) {
        props.afterUpdate()
        toast('Course updated successfully', { hideProgressBar: false, autoClose: 2000, type: 'success' })
        handleClose()
      } else {
        setLoader(false)
      ...
      >
        <Box sx={style}>
          ...
          <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between', }}>
            <h2 style={{
              color: 'var(--primary)',
            }} id="parent-modal-title">Update Course</h2>
            <DoneIcon
